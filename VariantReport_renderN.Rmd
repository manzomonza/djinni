...
datadisply = dplyr::select(datadisply, -COSMIC_n_tissue)
...
cosmic_heat = dplyr::select(gene, coding, protein, COSMIC_n_tissue)

#cosmic_heat$tissue = stringr::str_remove_all(cosmic_heat$COSMIC_n_tissue, " \\(\\d+\\)| ")
cosmic_heat$tissue = stringr::str_remove_all(cosmic_heat$COSMIC_n_tissue, pattern = " ")
cosmic_heat$tissue = stringr::str_split(cosmic_heat$tissue, pattern = ";")
cosmic_heat |>
  tidyr::unnest(cols = tissue) |>
  tidyr::separate(col = 'tissue',
                  into = c('tissue','No'), sep = "\\(") |>
  dplyr::mutate(No = gsub("\\)",'', No)) |>
  dplyr::filter(!is.na(tissue)) |>
  dplyr::select(gene, coding, protein, tissue, No) |> 
  dplyr::mutate(uid = paste0(gene,coding, protein),
                No = as.numeric(No)) |>
  ggplot(aes(x = forcats::fct_reorder(tissue, -No), y=forcats::fct_reorder(uid, No))) + 
